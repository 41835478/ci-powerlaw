...
class Task extends CI_Controller {
    ...
    public function index() {
        if (isset($_SESSION['user_id']) && $_SESSION['user_id'] != '') {
			...
			$getuserinfo = $this->AccountModel->getuserinfobyuserid($user_id);
            $data['all_cases'] = $this->CaseModel->ALLFopenC($getuserinfo->FirmId);
            $data['all_staff'] = $this->TaskModel->allstaff($user_id);
            $data['taskinfo'] = $this->TaskModel->alltask($user_id);
            $data['getalluser'] = $this->TaskModel->alluserinfofortask($user_id);
            //print_r($data['taskinfo']); exit; 
    ...
    public function editfronttask() {
        if (isset($_SESSION['user_id']) && $_SESSION['user_id'] != '') {
            $user_id = $_SESSION['user_id'];
            $this->load->model('CaseModel');
            $task_id = base64_decode($this->uri->segment('2'));
            $data['username'] = $this->TaskSystemModel->alluserinfofortask();
            $data['allcase'] = $this->TaskSystemModel->selectallopencase();
            $data['edittask'] = $this->TaskModel->getedittaskinfo($task_id);
            //echo '<pre>'; print_r($data['edittask']); exit; 
            //$this->load->view('admin_template/headerlink');
            $this->load->view('powerlawadmin/admin_template/headerlink');
            $this->load->view('admin_template/header');
    ...
    public function viewlawyertask() {
        if (isset($_SESSION['user_id']) && $_SESSION['user_id'] != '') {
            ...
            $data['allcase'] = $this->TaskSystemModel->selectallopencase();
            $data['edittask'] = $this->TaskModel->getedittaskinfo($task_id);
           //echo '<pre>'; print_r($data['edittask']); 
            //$this->load->view('admin_template/headerlink');
            $this->load->view('powerlawadmin/admin_template/headerlink');
            $this->load->view('admin_template/header');
 ...
 public function taskPdf() {
        ...
        if (!isset($userId) || $userId != true) {
            ...
            $data['taskinfo'] = $this->TaskModel->alltask($userId);
