                                        ...
                                        <tbody>
                                            <?php foreach ($taskinfo as $task) { ?>
                                                <tr>
                                                    ...
                                                    <td><a href="<?php echo base_url()?>viewlawyertask/<?php echo $task['TaskId']; ?>"><?php echo $task['TaskName']; ?></a></td>
                                                    <td><?php echo $priority; ?></td>
                                                    <td><?php  if($task['DueDate']!='1970-01-01 00:00:00'){echo date('d F, Y', strtotime($task['DueDate']));}else{echo "Not Set";}?></td>
                                                    
                                                    <td><a href="<?php echo base_url()?>cases/viewCase/<?php echo base64_encode($task['CaseId']); ?>">
                                                        <?php if($task['CaseId']){
                                                            foreach($all_cases as $a_cases){ 
                                                                if($task['CaseId']==$a_cases->CaseId){
                                                                   echo $a_cases->CaseName; 
                                                                }
                                                    ...
                                                    <td>
                                                        ...
                                                        $staffid=explode(",",$task['AssignedTo']);
                                                        foreach($staffid as $astaffid){
                                                            $userinfo = $this->TaskModel->getassigneduser($astaffid); ?>
                                                        <p> <a href="<?php echo base_url()?>contacts/users/<?php echo base64_encode($userinfo->id); ?>"><?php echo $userinfo->fullname; ?></a></p><?php 
                                                        }
                                                    ...
                                                    <td>
                                                        ...
                                                        <a href="<?php echo base_url() ?>editfronttask/<?php echo $task['TaskId']; ?>" title="Update" aria-label="Update" data-pjax="0"><span class="glyphicon glyphicon-pencil"></span></a> 
                                                        <a class="alertForDelete" href="<?php echo base_url() ?>deletelaywertask/<?php echo $task['TaskId']; ?>" title="Delete" aria-label="Delete" data-confirm="Are you sure you want to delete this item?" data-method="post" data-pjax="0"><span class="glyphicon glyphicon-trash"></span></a></td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                                    </table>
...
<script>
    ...
    function getallcase() {
        ...
        $.ajax({
            url: "<?php echo site_url('getacbyuserid'); ?>",
            type: "post",
...
alert('Ayan');
        $.ajax({
            url: "<?php echo site_url('getfrontalltaskbycaseinfo'); ?>",
            type: "post",
            data: {
                case_id: case_id,
            },
