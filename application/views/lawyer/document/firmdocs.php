                ...
                <div class="panel-body">
                    <div id="w0" data-pjax-container="" data-pjax-push-state data-pjax-timeout="1000" class="table-responsive"> 
                        <table class="table table-bordered" id="firmdatatables" style="border: 1px solid #ddd;  word-wrap:break-word;">
                            ...
                            <tbody>
                                <?php $i=1;foreach ($allfirmdoc as $doc) {                                        //print_r($doc); ?>
                                    <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><a><?php if($doc['OriginalName']){echo $doc['OriginalName'];} ?></a></td>
                                        <td><?php if($doc['CaseId']){foreach($allcase as $acase){if($doc['CaseId']==$acase->CaseId){?><a><?php echo $acase->CaseName;?></a><?php }}}?></td>
                                        <td>Updated On <?php echo date('d F, Y', strtotime($doc['UploadedOn'])); ?><p>By <?php if($doc['UploadedBy']){foreach($alluser as $auser){if($doc['UploadedBy']==$auser->id){?><a><?php echo $auser->fullname;?></a><?php }}}?></p></td>
                                        <td>
                                              <?php if ($_SESSION['permissionsession']['documents'] == '2' ||$_SESSION['permissionsession']['documents'] == '1') { ?>
                                            <a href="<?php echo base_url() ?>viewdocs/<?php echo $doc['DocumentId'];?>" title="Update" aria-label="Update" data-pjax="0"><span class="glyphicon glyphicon-eye-open"></span></a>
                                              <?php }?>
                                             <?php if ($_SESSION['permissionsession']['documents'] == '1') { ?>
...
//     if(document.getElementById("newstaffall")){
//        v= document.getElementById("newstaffall").value;
//     }
//
