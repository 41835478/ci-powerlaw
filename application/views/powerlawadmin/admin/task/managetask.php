                        ...
                        <div class="panel-body">
                            <div id="w0" data-pjax-container="" data-pjax-push-state data-pjax-timeout="1000" class="table-responsive"> 
                                <ul class="icons-list" style="float:right">
                                    <li>
                                        <a href="<?php echo base_url() ?>addTask"><span class="label label-success label-icon"><i class="glyphicon glyphicon-plus"></i></span></a>                                </li>
                                    <li class="dropdown">
                                        ...
                                        <tbody>
                                            <?php foreach ($taskinfo as $task) { ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                       <?php if ($_SESSION['permissionsession']['events'] == '2' || $_SESSION['permissionsession']['events'] == '1') { ?>
                                                        <a href="<?php echo base_url() ?>viewtask/<?php echo $task['TaskId']; ?>" title="Update" aria-label="Update" data-pjax="0"><span class="glyphicon glyphicon-eye-open"></span></a>
                                                       <?php } ?>
                                                         <?php if ($_SESSION['permissionsession']['events'] == '1') { ?>
                                                        <a href="<?php echo base_url() ?>edittask/<?php echo $task['TaskId']; ?>" title="Update" aria-label="Update" data-pjax="0"><span class="glyphicon glyphicon-pencil"></span></a> 
                                                        <?php if ($_SESSION['permissionsession']['delete_item'] == '1') { ?>
                                                        <a class="alertForDelete" href="<?php echo base_url() ?>deletetask/<?php echo $task['TaskId']; ?>" title="Delete" aria-label="Delete" data-confirm="Are you sure you want to delete this item?" data-method="post" data-pjax="0"><span class="glyphicon glyphicon-trash"></span></a>
                                                         <?php } ?>
                                                         <?php } ?>
                                                    </td>
                                                <?php } ?>
...
<script>
    ...
    function getallcase() {
        ...
        $.ajax({
            url: "<?php echo site_url('getallsitecaseinfo'); ?>",
            type: "post",
    ...
    function showtaskwithincom(criteria) {
        $.ajax({
            url: "<?php echo site_url('getalltaskbycriteria'); ?>",
            type: "post",
            data: {
                criteria: criteria,
            },
    ...
    $('#userdropdown').change(function () {
        ...
        $.ajax({
            url: "<?php echo site_url('getalltaskbyuserid'); ?>",
            type: "post",
            data: {
                userid: userid,
            },
    ...
    $('#groupbydropdown').change(function () {
        ...
        $.ajax({
            url: "<?php echo site_url('getalltaskbygroupby'); ?>",
            type: "post",
            data: {
                groupby: groupby,
            },
