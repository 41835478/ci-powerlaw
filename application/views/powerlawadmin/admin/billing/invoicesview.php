                ...
                <div class="panel-body">
                    <div id="w0" data-pjax-container="" data-pjax-push-state data-pjax-timeout="1000" class="table-responsive"> 
                        <table class="table table-bordered" id="firmdatatables" style="border: 1px solid #ddd;  word-wrap:break-word;">
                            ...
                            <tbody>
                                <?php foreach ($allinvoiceinfo as $invoice) {                                      //echo '<pre>';  print_r($invoice);  //exit;?>
                                    <tr>
                                        <td><?php echo $invoice['InvoiceId']; ?></td>
                                        <td><a href="<?php echo base_url()?>viewinvoicedetials/<?php echo $invoice['InvoiceId']; ?>"><span class="label label-primary"><p>View invoice #<?php echo $invoice['InvoiceId']; ?></p></span></a></td>
                                        <td><a><?php echo $invoice['fullname']; ?></a></td>
                                        <td><?php 
                                        $caseinfo = $this->CaseSystemModel->caseinfobyid($invoice['CaseId']);
                                        ?>
                                          <a href="<?php echo base_url()?>viewcaseadmin/<?php echo $caseinfo->CaseId; ?>"><?php echo $caseinfo->CaseName; ?></a></td>
                                        <td><a><?php echo $invoice['TotalAmount']; ?></a></td>
                                        <td><a><?php //echo $invoice['CaseName']; ?></a></td>
                                        <td><a><?php //echo $invoice['CaseName']; ?></a></td>
                                        <td>Created On <?php echo date('d F, Y', strtotime($invoice['CreatedOn'])); ?><p>By <a><?php echo $invoice['fullname']; ?></a></p></td>
                                        <?php if($invoice['InvoiceStatus'] =='1') { ?>
                                        <td><span class="label label-warning">Create</span></td>
                                        <?php } else if($invoice['InvoiceStatus'] =='2') {?>
                                        <td><span class="label label-primary">View</span></td>
                                        <?php } else if($invoice['InvoiceStatus'] =='3') {?>
                                        <td><span class="label label-success">Sent</span></td>
                                        <?php } ?>
                                        <td><a><?php //echo $invoice['CaseName']; ?></a></td>
                                        <td>
                                            <a href="<?php echo base_url() ?>editfirm/<?php //echo $firm['FirmId'];        ?>" title="Update" aria-label="Update" data-pjax="0"><span class="glyphicon glyphicon-eye-open"></span></a>
                                            <a class="alertForDelete" href="<?php //echo base_url()        ?>deletefirm/<?php //echo $firm['FirmId'];        ?>" title="Delete" aria-label="Delete" data-confirm="Are you sure you want to delete this item?" data-method="post" data-pjax="0"><span class="glyphicon glyphicon-trash"></span></a>
                                        </td>
                                    </tr>
                                <?php } ?>
                            </tbody>
